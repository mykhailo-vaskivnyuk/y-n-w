...
metatests.test('Pool: wait for release', async (test) => {
  ...
  const { release: release1 } = await pool.get();
  const { release: release2 } = await pool.get();

  pool.get().then(({ item: item3 }) => {
    test.strictSame(item3, obj2);
  });
  pool.get().then(({ item: item4 }) => {
...
//   const { item: item1 } = await pool.get();
//   test.strictSame(item1, obj1);
//   const { item: item2, release: release2 } = await pool.get();
//   test.strictSame(item2, obj2);
...
//   await pool.get(true);
//   const { item: item2 } = await pool.get();
