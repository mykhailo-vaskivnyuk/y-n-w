...
export const decorateAsync = <T extends { new (...args: any[]): any }>(
  target: T,
  methodName: string,
  ...
  if (isAsync(method) || isAsync(operation)) {
    target.prototype[methodName] = async function (...args: Parameters<typeof method>) {
      let result;
  ...
  target.prototype[methodName] = function (...args: Parameters<typeof method>) {
